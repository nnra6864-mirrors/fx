...
const SourceInventory = struct {
    ...
    fn detect(alloc: Allocator, ctx: ?*anyopaque, probe: SourceProbeFn, cancelled: ?*const std.atomic.Value(bool)) !SourceInventory {
...
const InventoryRefreshTask = struct {
    ...
    fn workerMain(self: *InventoryRefreshTask) void {
        ...
        self.inventory = SourceInventory.detect(self.alloc, self.deps.ctx, self.deps.probe, &self.cancel_requested) catch |err| {
            self.failure = err;
            return;
        };
    }
...
pub const Runtime = struct {
    ...
    fn refreshSourceInventoryWithProbe(
        ...
        self.applySourceInventory(try SourceInventory.detect(alloc, ctx, probe, null));
    }
