...
These are intentionally red implementation tests plus passing deterministic
reference simulations. They are the development worklist for journaled execution,
not an implementation of it. They must not be converted to expected-failure tests
or made green by weakening recovery guards.
...
The proposal is not a released API. SDK witnesses supply its proposed `journal`,
`onEntry`, `requestId`, tool `replay`, identity and draft-key fields. The current
SDK silently ignores several of them; tests report absent boundaries explicitly,
not as import errors or timeouts.
...
runs all selected layers even when an earlier one fails. It exits nonzero while
any assertion is red. Logs are under `.zig-cache/journal-results/`.
...
FX_E2E_DISABLE_DOTENV=1 FX_JOURNAL_RED=1 bun test tests/e2e/session-recovery.test.ts --test-name-pattern 'journal witness native crash'
```
...
Red assertions cover the original selected call before its effect, A's result
...
The persistence capture/restore seam currently uses `RecoveryCheckpointEffect`.
When adding the typed execution journal and replay policy, update that seam to
consume the new authoritative records. Do not rewrite the behavioral assertions.
The safe-effect fixture must be explicitly opted into the new typed replay policy;
ordinary shell calls must remain blocked. No production persistence or replay
behavior is added by these tests.

`zig build test-journal` is a separate opt-in root. The deliberately red tests are
not imported into the normal binary's unit-test root.
...
The test is opt-in via `FX_JOURNAL_RED=1` while red. Its existing PGSO owner remains
appropriate for recovery correctness; no new root E2E file or unclassified corpus
owner was added. This is not a replacement for the existing safe-upgrade, session
...
Until journal entries exist, recreation uses the newest **actual** checkpoint
written by the current core, never modified checkpoint bytes. Once entries exist,
recreation is journal-only. Effects and local receipts commit in one fixture
...
| J06 blocked single/batch | Zig fail-closed control; Native/Wasm pending state, resume, abandon | `unknown` A / `skipped` B projection simulation; real transcript API still absent |
...
| J12 identities | Zig original call; Native/Wasm request/turn/call context and repeated restoration | Real transcript message-ID projection remains unqualified |
...
This worklist distinguishes failures already observable in today's runtime from
proposed entry barriers that do not exist yet. Multiple boundary tests currently
fail at the same missing `onEntry` prerequisite; their downstream assertions
become reachable as implementation proceeds. The core witnesses do not depend
on those missing JavaScript fields.
