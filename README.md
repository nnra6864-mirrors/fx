...
While fx is working, Ctrl+C clears a nonempty composer without interrupting the turn. Press Ctrl+C again with an empty composer to cancel the main agent's active work. Delegated children keep running until they finish, the main agent explicitly cancels them, or fx exits.
...
The `subagent` tool's `run` operation delegates one temporary task, and `message` creates or continues a named persistent agent. A first message creates the named child immediately; optional instructions set or replace that child's system overlay while preserving fx's trusted base prompt. Child sessions remain private to their saved parent session.

In the interactive shell, you can steer the main agent while its children work. A call normally waits for its result; new input can release that wait with a truthful still-running acknowledgement. The child continues with the same task and identity. Its eventual result appears separately, and the main agent can use it at its next safe boundary. Full requests and child evidence remain available in Ctrl+O's full transcript. Other tools in the same batch may still need to finish before the main agent can respond.

Stopping or pausing the main agent, reaching its step limit, attaching an image, or changing the selected session does not cancel its children. Results stay with the original session. Completion does not restart a stopped main task or appear in another session; retained results are available when you next use the owning session. To stop a child intentionally, ask the main agent to cancel it. The main agent's `cancel` operation targets an exact child and work ID, not whichever task happens to be newest. Quitting fx stops and joins all its children; they are not background daemons. Children still obey their own limits, failures and security checks. Human permission decisions remain available, while dismissing a child prompt does not deny its request.

Noninteractive `fx ask`, ACP, SDK and child callers keep their existing synchronous run/message behavior; the independent-child cancellation operation is unavailable there.

Failed calls include the captured failure reason and any partial result, including HTTP failures before an answer or after earlier tool calls. Earlier tool effects are not rolled back or automatically retried. Existing child records remain readable, but records saved by this version cannot be reopened by older binaries that only support child registry schema 1. Sessions that record independent child-work delivery also use a newer conversation format and cannot be reopened by versions without that support. Existing sessions without the new records retain their supported formats; restarting fx does not automatically restart interrupted child work.
